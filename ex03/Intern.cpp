...
AForm *Intern::makeForm(const std::string &name, const std::string &target) {
  std::string form_types[3] = {"robotomy request", "presidential pardon",
                               "shrubbery creation"};
  AForm *(*form_generators[3])(const std::string &) = {
      &makeRobotomy, &makePardon, &makeShrubbery};
  ...
  if (selected_form_generator == NULL) {
    std::cerr << "Can't create a form named \"" << name
